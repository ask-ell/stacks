...
export type ErrorSetter<FormData> = (
  key: keyof FormData
...
export type OnSubmitCallback<FormData> = (
  params: OnSubmitCallbackParams<FormData>
) => void;
...
export function useFormController<FormData>({
  defaultValue,
  onSubmit,
  submitMode,
}: UseFormParams<FormData>): FormController<FormData> {
  const [formData, setFormData] = useState<FormData>(defaultValue);
  const [errors, setErrors] = useState<ErrorsMap<FormData>>(new Map());

  const appendError: ErrorSetter<FormData> =
    (key: keyof FormData) =>
    (message: string): void => {
  ...
  const updateFormData: FormDataFieldUpdater<FormData> =
    <Key extends keyof FormData>(key: Key) =>
    (value: FormData[Key]) => {
      setTimeout((): void => {
        ...
        if (submitMode === SubmitMode.AUTOMATIC) {
          prepareOnSubmit((): void =>
            onSubmit({ formData: newFormData, appendError })
          );
        }
      });
    };

  const submit: SubmitTrigger = (): void =>
    prepareOnSubmit((): void => onSubmit({ formData, appendError }));
