...
export type KeyvRowData<EntityState extends AggregateRootState> = Omit<
  EntityState,
  'id'
>;

export abstract class KeyvAggregateRootRepository<
    EntityState extends AggregateRootState
  >
  extends KeyvClient<EntityState>
  implements IAggregateRootRepository<EntityState>
{
  ...
  protected abstract purgeData(
    dto: KeyvRowData<EntityState>
  ): KeyvRowData<EntityState>;

  private isAlreadySaved(entityState: EntityState): Promise<boolean> {
    return this.instance.has(entityState.id);
  }

  private persist({ id, ...dto }: EntityState): Promise<boolean> {
