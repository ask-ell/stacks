...
import {
  K8SClusterParams,
  PersistClusterCustomObjectDTO,
  K8SSecret,
  K8SService,
  K8SCustomResource,
  DeleteClusterCustomObjectDTO,
...
export class K8SCluster implements IK8SCluster {
  ...
  constructor({ configFilePath, logger }: K8SClusterParams) {
    this.logger = logger;
  ...
  async isNamespacePersisted(name: string): Promise<boolean> {
    ...
    return allNamespaces.some(
      (namespace: K8SNamespace): boolean => namespace.metadata?.name === name
    );
  }
  ...
  async createDeployment({
    namespace,
    dto,
  }: CreateDeploymentDTO): Promise<void> {
  ...
  async deleteDeployment({
    name,
    namespace,
  }: DeleteComponentDTO): Promise<void> {
  ...
  async getServiceAccountsFromNamespace(
    namespace: string
  ...
  async createServiceAccount({
    name,
    namespace,
  }: CreateServiceAccountDTO): Promise<void> {
  ...
  async createServiceAccountToken({
    namespace,
    name,
  }: CreateServiceAccountTokenDTO): Promise<void> {
  ...
  async getRoles({ namespace }: GetRolesDTO): Promise<K8SRole[]> {
    return getItems(
      this.rbacAuthorizationV1Api.listNamespacedRole({ namespace })
    );
  }
  ...
  async getRoleBindings({
    namespace,
  }: GetRoleBindingsDTO): Promise<K8SRoleBinding[]> {
    return getItems(
      this.rbacAuthorizationV1Api.listNamespacedRoleBinding({ namespace })
    );
  }
  ...
  async createRoleBinding({
    ...
    namespace,
    subjects,
  }: CreateRoleBindingDTO): Promise<void> {
  ...
  async getAllCustomResourceDefinitions(): Promise<
    K8SCustomResourceDefinition[]
  > {
  ...
  async createCustomResourceDefinition(
    body: K8SCustomResourceDefinition
  ): Promise<void> {
    await this.apiextensionsV1Api
      .createCustomResourceDefinition({ body })
      .catch(this.logger.error);
  }

  async getAllClusterCustomObjectsFromResourceDefinition<
    CustomResource extends K8SCustomResource
  >({
    spec: {
      group,
      names: { plural },
    ...
    const { items }: ComponentsFetchingResponse<CustomResource> =
  ...
  async deleteClusterCustomObject<CustomResource extends K8SCustomResource>({
    resourceDefinition,
    metadata,
  }: DeleteClusterCustomObjectDTO<CustomResource>): Promise<void> {
