...
export const success = <T>(data: T): IResult<T> =>
  new Result(data, null) as IResult<T>;
...
export function mergeResults<
  MergedResultData extends unknown[],
  MergedResultDataRange extends IResult[]
>(results: MergedResultDataRange): IResult<[...MergedResultData]> {
...
export const resultFromSnapshot = <T>({
  successValue,
  failValue,
}: ResultSnapshot<T>): IResult<T> =>
  new Result(successValue, failValue) as IResult<T>;
...
export const fromPromiseToResult = async <T>(
  promise: () => Promise<T>
): Promise<IResult<T>> => {
  try {
